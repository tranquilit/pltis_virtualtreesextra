...
uses
     ...
     Messages, VirtualTrees, Classes, Controls, SysUtils, Graphics, StdCtrls,
...
type
  ...
  private
    ...
    procedure WMChar(var Message: TLMChar); message WM_CHAR;
...
begin
        ...
        FRangeErrorText := 'Value must be in range of %d - %d!';
        FValidityErrorText := '"%s" is not a valid number!';
        FErrorTitle := 'Error...';
end;
...
procedure TVirtualPropertyTree.DoAfterItemPaint(Canvas: TCanvas; Node: PVirtualNode; const ItemRect: TRect);
begin
     with Canvas do
     begin
          ...
          if (vsHasChildren in Node.States) and
            ...
            PaintNodeButton( Canvas, Node, 2, ItemRect, 3, 3, bdLeftToRight );
     end;
     // inherited; //
end;

procedure TVirtualPropertyTree.DoGetText(Node: PVirtualNode; Column: TColumnIndex; TextType: TVSTTextType; var Text: String);
begin
     if (Node^.Parent = RootNode) then
     begin
          if Column = 0 then
...
begin
     NodeChildCount := Length( FCategories[Node^.Index].Fields );
...
function TVirtualPropertyTree.DoPaintBackground(Canvas: TCanvas; const R: TRect): Boolean;
begin
     with Canvas do
     begin
          Pen.Color := self.Colors.GridLineColor;
...
procedure TVTPropertyEdit.WMChar(var Message: TLMChar);
...
procedure TVTPropertyEdit.WMKeyDown(var Message: TLMKeyDown);
begin
...
procedure TVirtualPropertyTree.DoNewText(Node: PVirtualNode; Column: TColumnIndex; const Text: String);
begin
...
begin
     ...
     begin
          case DataType of
            dtInteger:  try
                          t := StrToInt( Text );
                          if (t >= MinIntValue) and (t <= MaxIntValue) then
                            PInteger(Data)^ := t
                          else
            ...
            dtExtended: try
                          ...
                          if (x >= MinExtValue) and (x <= MaxExtValue) then
                            PExtended(Data)^ := x
                                ...
                                if PropertyTreeOptions.ShowRangeError then MessageBox( Handle, PChar(Format(StringReplace(PropertyTreeOptions.RangeErrorText, '%d', '%g', [rfReplaceAll, rfIgnoreCase]), [MinExtValue, MaxExtValue])), PChar(PropertyTreeOptions.ErrorTitle), mb_ok or MB_ICONEXCLAMATION );
                        except
                                if PropertyTreeOptions.ShowValidityError then MessageBox( Handle, PChar(Format(PropertyTreeOptions.ValidityErrorText, [Text] )), PChar(PropertyTreeOptions.ErrorTitle), mb_ok or MB_ICONEXCLAMATION );
                        end;
          end;
...
procedure TVirtualPropertyTree.WMLButtonDown(var Message: TLMLButtonDown);
