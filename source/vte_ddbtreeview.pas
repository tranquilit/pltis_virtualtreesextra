...
uses
  ...
  Variants,
  SysUtils, Messages, Classes, Controls, VirtualTrees, DB, LCLIntf, LCLType, LMessages, Contnrs,
  Menus,
  {$ifdef Windows}
  ActiveX,
  {$else}
  FakeActiveX,
  {$endif}
  ComCtrls, ImgList, Types, Forms, vte_ddbtreeviewutils;
...
type
  ...
  TVirtualDDBTreeView = class;

  TVirtualDDBTreeViewLink = class(TDataLink)
  private
    FDBTree: TVirtualDDBTreeView;
  protected
  ...
  public
   property DBTree: TVirtualDDBTreeView read FDBTree;
   constructor Create(ADBTree: TVirtualDDBTreeView);
  end;

  TVirtualDDBTreeFields = class(TPersistent)
  private
    FDBTreeView: TVirtualDDBTreeView;
    FKeyFieldName: String;
    FParentFieldName: String;
    FParentOfRootValue: String;
    FListFieldName: String;
    FHasChildrenFieldName: String;
    function GetDBTreeView: TVirtualDDBTreeView;
  ...
  public
    ...
    property DBTreeView: TVirtualDDBTreeView read GetDBTreeView;
  published
  ...
  TVirtualDDBTreeImages = class(TPersistent)
  private
  ...
  TOnDBTreeCreateNode = procedure(Sender: TVirtualDDBTreeView; Node: PVirtualNode;
    DataSet: TDataSet) of object;
  TDTOnGetFieldValue = procedure(ADataSet: TDataSet;
    var HasChildrenValue: Boolean) of object;

  TDTVirtualDDBTreeColumn = class(TVirtualTreeColumn)
  private
    FFieldName: String;
  ...
  TVirtualDDBTreeView = class(TVirtualStringTree)
  private
    FDataLink: TVirtualDDBTreeViewLink;
    FDraggedNode: PVirtualNode;
    FDBTreeFields: TVirtualDDBTreeFields;
    FDBTreeImages: TVirtualDDBTreeImages;
    FOnDBTreeCreateNode: TOnDBTreeCreateNode;
    FIntOperationCounter: Integer;
    FOnGetHasChildren: TDTOnGetFieldValue;
    FUseFilter: Boolean;
    procedure SetDBTreeImages(const Value: TVirtualDDBTreeImages);
    function GetDataSource: TDataSource;
    function IsDataAvail: boolean;
    function NodeCaption(ANode: PVirtualNode): String;
    procedure SetDBTreeFields(const Value: TVirtualDDBTreeFields);
    procedure DoGoUp;
    procedure DoGoDown;
  ...
  protected
    ...
    procedure DoDragDrop(Source: TObject; DataObject: IDataObject; Formats: TFormatArray;
                         Shift: TShiftState; const Pt: TPoint; var Effect: DWORD; Mode: TDropMode); override;
    function  DoDragOver(Source: TObject; Shift: TShiftState; State: TDragState; const Pt: TPoint; Mode: TDropMode;
                        var Effect: DWORD): Boolean; override;
  ...
  published
    ...
    property DBTreeFields: TVirtualDDBTreeFields read FDBTreeFields write SetDBTreeFields;
    property DBTreeImages: TVirtualDDBTreeImages read FDBTreeImages write SetDBTreeImages;
...
function TVirtualDDBTreeView.CreateChild(ANode: PVirtualNode): PVirtualNode;
    ...
    with Header.Columns do
      ...
      begin
        ...
        for i := 0 to Count - 1 do
          with Items[i] as TDTVirtualDDBTreeColumn do
            if FieldName <> '' then
...
procedure TVirtualDDBTreeView.CheckRecordDeleted;
var
...
function TVirtualDDBTreeView.ChildrenPresent(Node: PVirtualNode): Boolean;
var
  ListNode: TDTNodeItem;
  Res: variant;
  BMark: TBytes;  // ct9999
...
constructor TVirtualDDBTreeView.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  FDBTreeFields := TVirtualDDBTreeFields.Create;
  FDBTreeFields.FDBTreeView := Self;
  FDBTreeImages := TVirtualDDBTreeImages.Create;
  FNodes := TDTNodeList.Create;
  FDataLink := TVirtualDDBTreeViewLink.Create(Self);
...
procedure TVirtualDDBTreeView.DataLinkDataSetScrolled;
...
destructor TVirtualDDBTreeView.Destroy;
begin
...
procedure TVirtualDDBTreeView.DoChange(Node: PVirtualNode);
begin
...
function TVirtualDDBTreeView.DoGetImageIndex(Node: PVirtualNode; Kind: TVTImageKind;
...
procedure TVirtualDDBTreeView.DoGetText(Node: PVirtualNode; Column: TColumnIndex; TextType: TVSTTextType; var Text: String);
var
  Item: TDTNodeItem;
begin
...
function TVirtualDDBTreeView.GetNodeByID(ID: variant): PVirtualNode;
var
  Index: Integer;
begin
  Result := nil;
...
procedure TVirtualDDBTreeView.CreateRootNodes;
begin
  BeginUpdate;
  Clear;
  if FCloneDataSet <> nil then
...
function TVirtualDDBTreeView.NodeCaption(ANode: PVirtualNode): String;
var
  Data: PNodeItem;
begin
  Result := '';
...
procedure TVirtualDDBTreeView.DataLinkRecordChanged(AField: TField);
var
  Data: PNodeItem;
...
procedure TVirtualDDBTreeView.SetDataSource(const Value: TDataSource);
begin
...
{ TVirtualDDBTreeViewLink }

procedure TVirtualDDBTreeViewLink.ActiveChanged;
var 
  CurCursor: TCursor;
begin
  with FDBTree do
...
constructor TVirtualDDBTreeViewLink.Create(ADBTree: TVirtualDDBTreeView);
begin
 inherited Create;
 FDBTree := ADBTree;
 VisualControl := True;
...
procedure TVirtualDDBTreeViewLink.DataSetChanged;
begin
  inherited;
  FDBTree.DataLinkDataSetChanged;
end;

procedure TVirtualDDBTreeViewLink.DataSetScrolled(Distance: Integer);
begin
  with FDBTree do
    DataLinkDataSetScrolled;
end;

procedure TVirtualDDBTreeViewLink.RecordChanged(Field: TField);
begin
  inherited;
  with FDBTree do
...
procedure TVirtualDDBTreeView.DoDragDrop(Source: TObject; DataObject: IDataObject; Formats: TFormatArray;
...
procedure TVirtualDDBTreeView.DoStartDrag(var DragObject: TDragObject);
begin
  if not (csDesigning in ComponentState) then
    FDraggedNode := FocusedNode;
end;

function TVirtualDDBTreeView.DoDragOver(Source: TObject; Shift: TShiftState; State: TDragState; const Pt: TPoint; Mode: TDropMode;
                        var Effect: DWORD): Boolean;
begin
...
procedure TVirtualDDBTreeView.CheckRecordInserted;
var
...
procedure TVirtualDDBTreeView.DestroyCloneDataSet;
begin
  if FCloneDataSet <> nil then
  begin
    FCloneDataSet.Close;
...
procedure TVirtualDDBTreeView.Notification(AComponent: TComponent;
...
procedure TVirtualDDBTreeView.UpdateCloneDataSet;
begin
  if FCloneDataSet <> nil then
...
procedure TVirtualDDBTreeView.SetCloneDataSetFilter(AParentValue: String);
begin
  if FCloneDataSet <> nil then
    FCloneDataSet.Filter := GetFilterString(AParentValue);
end;

procedure TVirtualDDBTreeView.SetDBTreeFields(const Value: TVirtualDDBTreeFields);
begin
  FDBTreeFields := Value;
end;

procedure TVirtualDDBTreeView.SetDBTreeImages(const Value: TVirtualDDBTreeImages);
begin
  FDBTreeImages := Value;
end;

procedure TVirtualDDBTreeView.DoFreeNode(Node: PVirtualNode);
var
  Index: Integer;
  ListNode: TDTNodeItem;
begin
...
function TVirtualDDBTreeView.RecordHasChildren: Boolean;
var
  BMark: TBytes; /// ct9999
  WasFiltered: Boolean;
  KeyValue: Variant;
...
function TVirtualDDBTreeView.GetListNode(Node: PVirtualNode): TDTNodeItem;
var
  CurData: ^TObject;
begin
  Result := nil;
...
{ TVirtualDDBTreeFields }

function TVirtualDDBTreeFields.FieldToKey(AFieldName: String;
...
function TVirtualDDBTreeFields.GetDBTreeView: TVirtualDDBTreeView;
begin
  Result := FDBTreeView; 
end;

procedure TVirtualDDBTreeView.KeyDown(var Key: Word; Shift: TShiftState);
begin
end;

procedure TVirtualDDBTreeFields.SetKeyFieldName(const Value: String);
begin
...
procedure TVirtualDDBTreeFields.SetListFieldName(const Value: String);
begin
...
procedure TVirtualDDBTreeFields.SetParentFieldName(const Value: String);
begin
...
procedure TVirtualDDBTreeFields.SetParentOfRootValue(const Value: String);
begin
...
procedure TVirtualDDBTreeView.Clear;
begin
  if FNodes <> nil then
    FNodes.ClearItems;
  inherited Clear;
...
procedure TVirtualDDBTreeView.SelectNode(Node: PVirtualNode);
begin
...
function TVirtualDDBTreeFields.ValidFieldName(AFieldName: String): Boolean;
var
  i: Integer;
  FieldNames: TStringDynArray;
begin
...
//==================== TVirtualDDBTreeImages ==================================

constructor TVirtualDDBTreeImages.Create;
begin
...
procedure TVirtualDDBTreeView.DoNewText(Node: PVirtualNode; Column: TColumnIndex; const Text: String);
begin
...
function TVirtualDDBTreeView.CanGoDown: Boolean;
begin
  Result := RecordHasChildren;
end;

function TVirtualDDBTreeView.CanGoUp(ADataSet: TDataSet = nil): Boolean;
begin
  Result := False;
  if ADataSet = nil then
...
function TVirtualDDBTreeView.EndOfTree: Boolean;
begin
  Result := FEndOfTree;
end;

procedure TVirtualDDBTreeView.GoDown;
begin
  if CanGoDown then
  begin
    UpdateCloneDataSet;  
...
procedure TVirtualDDBTreeView.GoTop;
begin
  with FCloneDataSet, FDBTreeFields do
  begin
...
procedure TVirtualDDBTreeView.GoUp;
begin
  if CanGoUp then
  begin
    UpdateCloneDataSet;
...
procedure TVirtualDDBTreeView.Next;
...
procedure TVirtualDDBTreeView.DoGoDown;
begin
  with FCloneDataSet, FDBTreeFields do
  begin
...
procedure TVirtualDDBTreeView.DoGoUp;
var
  ParentValue: Variant;
begin
  with FDBTreeFields do
...
procedure TVirtualDDBTreeView.UpdateDataSet;
begin
  with FDBTreeFields do
...
procedure TVirtualDDBTreeView.UpdateText;
var
  Node: PVirtualNode;
  Data: PNodeItem;
begin
...
function TVirtualDDBTreeView.GetFilterString(AParentValue: String): String;
var
  i: Integer;
...
procedure TVirtualDDBTreeView.SetOnDBTreeCreateNode(
  const Value: TOnDBTreeCreateNode);
begin
  FOnDBTreeCreateNode := Value;
end;

procedure TVirtualDDBTreeView.SetOnGetHasChildren(const Value: TDTOnGetFieldValue);
begin
  FOnGetHasChildren := Value;
end;

function TVirtualDDBTreeView.GetColumnClass: TVirtualTreeColumnClass;
begin
  Result := TDTVirtualDDBTreeColumn;
end;

function TVirtualDDBTreeView.IsDataAvail: boolean;
begin
...
//================== TDTVirtualDDBTreeColumn ====================================

procedure TDTVirtualDDBTreeColumn.SetFieldName(const Value: String);
begin
  FFieldName := Value;
end;

procedure TVirtualDDBTreeView.DataLinkDataSetChanged;
begin
...
procedure TVirtualDDBTreeView.KeyChanged(AKey: String);
begin
  try
    with FDataLink.DataSet, FDBTreeFields do
...
function TVirtualDDBTreeView.GetInternalOperation: Boolean;
begin
  Result := FIntOperationCounter > 0;
end;

procedure TVirtualDDBTreeView.SetInternalOperation(const Value: Boolean);
begin
  if Value then
...
function TVirtualDDBTreeView.KeyToVarArray(Key: String): Variant;
var
  i: Integer;
...
function TVirtualDDBTreeView.ValidParent(AParentValue: String): Boolean;
var
  ParentKey: String;
begin
  with DBTreeFields do
...
function TVirtualDDBTreeView.VarArrayToKey(AArray: Variant): String;
var
  i: Integer;
  Key: String;
begin
...
function TVirtualDDBTreeView.TreeFieldChanged(ATreeField: String; AField: TField): Boolean;
var
  i: Integer;
...
procedure TVirtualDDBTreeView.NodeChanged(ANode: PVirtualNode);
var
  ListNode: TDTNodeItem;
begin
  with FDataLink.DataSet, FDBTreeFields do
