...
uses
  ...
  SysUtils, Messages, Classes, Controls, VirtualTrees, vte_edittree_editors,
  StdCtrls;
...
type
  ...
  TVirtualEditableColumn = class(TVirtualTreeColumn, IUnknown, IComboCustomDraw)
  private
    FEditable: Boolean;
...
function TVirtualEditableColumn.QueryInterface(constref IID: TGUID; out Obj): HResult; {$IFNDEF WINDOWS}cdecl{$ELSE}stdcall{$ENDIF};
begin
  if GetInterface(IID, Obj) then Result := S_OK else Result := E_NOINTERFACE;
end;

function TVirtualEditableColumn._AddRef: Integer;{$IFNDEF WINDOWS}cdecl{$ELSE}stdcall{$ENDIF};
begin
  Result := -1;
end;

function TVirtualEditableColumn._Release: Integer; {$IFNDEF WINDOWS}cdecl{$ELSE}stdcall{$ENDIF};
begin
  Result := -1;
end;
...
procedure TCustomVirtualEditTree.WMChar(var Message: TLMChar);
begin
  with Message do
    if (CharCode in [Ord(^H), 32..255] -
      ...
      if EditNode(FocusedNode, FocusedColumn) and Assigned(EditLink) then
      begin
