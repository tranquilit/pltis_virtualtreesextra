...
uses
  ...
  StdCtrls, VirtualTrees, Buttons, ExtCtrls, ComCtrls, Spin,EditBtn,
  TypInfo;
...
function TIntfOwnedPersistent.QueryInterface(constref IID: TGUID; out Obj): HResult;{$IFNDEF WINDOWS}cdecl{$ELSE}stdcall{$ENDIF};
begin
...
function TIntfOwnedPersistent._AddRef: Integer; {$IFNDEF WINDOWS}cdecl{$ELSE}stdcall{$ENDIF};
begin
...
function TIntfOwnedPersistent._Release: Integer; {$IFNDEF WINDOWS}cdecl{$ELSE}stdcall{$ENDIF};
begin
  if FOwnerInterface <> nil then
...
function TCustomEditLink.BeginEdit: Boolean;  stdcall;
begin
...
begin
  case Message.Msg of
    WM_CHAR:
      if not (TLMChar(Message).CharCode in [VK_ESCAPE, VK_TAB]) then
        FOldWndProc(Message);
    WM_GETDLGCODE:
  ...
  case Message.Msg of
    WM_KEYDOWN:
      case TLMKeyDown(Message).CharCode of
        VK_UP:   TLMKeyDown(Message).CharCode := VK_LEFT;
        VK_DOWN: TLMKeyDown(Message).CharCode := VK_RIGHT;
      end;
  end;
  inherited;
end;
...
begin
  inherited;
  //TDateEdit(EditControl).Kind := FKind;
end;
...
begin
  ...
  try
    if FDateSeparator <> #0 then SysUtils.DateSeparator := FDateSeparator;
      if FDateFormat <> '' then
        Result := FormatDateTime(FDateFormat, TDateEdit(EditControl).Date)
      else
        case FKind of
          dtkDate: Result := DateToStr(TDateEdit(EditControl).Date);
          dtkTime: Result := TimeToStr(TDateEdit(EditControl).Date);
        end;
  finally
...
begin
  ...
  try
    ...
    try
      ...
      if FDateSeparator <> #0 then SysUtils.DateSeparator := FDateSeparator;
      case FKind of
        dtkDate: TDateEdit(EditControl).Date := StrToDate(Value);
        dtkTime: TDateEdit(EditControl).Date := StrToTime(Value);
      end;
    finally
      ShortDateFormat := DF;
      SysUtils.DateSeparator := Sep;
...
type
  ...
  private
   ...
   // procedure WMLouseActivate(var Message: TLMMouseActivate); message WM_MOUSEACTIVATE;
...
procedure TSizeGrip.WMLButtonDown(var Message: TLMLButtonDown);
begin
  inherited;
  FCaptured := True;
...
procedure TSizeGrip.WMLButtonUp(var Message: TLMLButtonUp);
begin
  inherited;
  FCaptured := False;
...
begin
  Message.Result := 0;//LA_NOACTIVATE;
end;
   }
procedure TSizeGrip.WMMouseMove(var Message: TLMMouseMove);
var
  NewW, NewH: integer;
begin
  inherited;
...
procedure TGripMemo.CNCommand(var Message: TLMCommand);
begin
  inherited;
  with Message do
    if //(NotifyCode = EN_CHANGE) or
       (NotifyCode = EN_UPDATE) or
       (NotifyCode = ES_AUTOHSCROLL) or
       (NotifyCode = ES_AUTOVSCROLL) then
    begin
      Invalidate;
      FGrip.Invalidate;
    end;
...
begin
  ...
  case Message.Msg of
    WM_NOTIFY:
      if TLMNotify(Message).NMHdr.code = EN_UPDATE then
        FGrip.Invalidate;
  end;
end;
